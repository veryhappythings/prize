# Review Monster — Implementation Plan
...
- `src/config.ts` — read env vars, define cache dir `~/.review-monster`
...
- Cache dir: `~/.review-monster/<owner>-<repo>-<pr_number>/`
...
PR URL → parse-url → { owner, repo, number }
  ...
  → pipeline/generate → ~/.review-monster/<id>/site/index.html
...
2. Verify cache files appear in `~/.review-monster/`
